...
import { View, Text, StyleSheet, Image, Button, TextInput, ScrollView, ActivityIndicator } from "react-native";
...
const Profile = ({ userInfo }) => {
  ...
  const handleUpdateProfilePic = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/user/${userInfo._id}/profile-pic`, {
        method: "POST",
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.value}>@{userInfo.username || "Not available"}</Text>
        ...
        {uploads.length === 0 ? (
          ...
          uploads.map((post, index) => (
            <Text key={index} style={styles.postItem}>• {post}</Text>
          ))
        )}
